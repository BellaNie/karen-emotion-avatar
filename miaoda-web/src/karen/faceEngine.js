...
function facePriority(result, video) {
  ...
  if (!box || !video.videoWidth || !video.videoHeight) return area;
...
export function createFaceEngine({ video }) {
  ...
  async function startCamera() {
    if (!navigator.mediaDevices?.getUserMedia) {
      return { ok: false, error: new Error('camera-unsupported'), reason: 'unsupported' };
    }
    try {
      ...
      return { ok: false, error, reason: 'denied' };
    }
  }
