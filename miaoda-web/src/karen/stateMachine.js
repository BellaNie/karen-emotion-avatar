...
function isFaceInInteractionArea(face, diagnostics) {
  if (!face?.box || !diagnostics?.videoWidth || !diagnostics?.videoHeight) return false;
  const videoArea = diagnostics.videoWidth * diagnostics.videoHeight;
  const faceArea = face.box.width * face.box.height;
  if (faceArea / videoArea < DETECTION.minFaceAreaRatio) return false;
...
export function createEmotionStateMachine({ faceEngine, emotionAnalyzer, avatarPlayer, counter, ui, debugPanel }) {
  ...
  function updateDebug() {
    debugPanel.update({ state, face: currentFace, probabilities: latestProbabilities, analysis: latestAnalysis });
  }
  ...
  function setFace(face) { currentFace = face; updateDebug(); }
  ...
  function scheduleLoop() {
    ...
    loopTimer = setTimeout(() => { detectionTick(); }, DETECTION.pollMs);
  }

  async function detectionTick() {
    if (!running || state !== STATES.IDLE) return;
    if (!faceEngine.ready) { scheduleLoop(); return; }
    ...
    if (interactiveFace) {
      ...
      if (now() - presenceStartedAt >= DETECTION.presenceMs) { await observe(); return; }
    } else {
      presenceStartedAt = null;
    }
    scheduleLoop();
  ...
  async function observe() {
    if (!running || state !== STATES.IDLE || !faceEngine.ready) return;
    setState(STATES.OBSERVING);
    ui.setMessage(COPY.observing);
    setAnalysis(emotionAnalyzer.emptyAnalysis());
    avatarPlayer.play('observing', { loop: true, muted: true, fallbackMs: OBSERVING_FALLBACK_MS });
    ...
    if (result.cancelled) { await startIdle(); return; }
    setAnalysis(result.analysis);
    if (!result.analysis?.validSampleCount || !result.analysis?.winner) { await startIdle(); return; }
  ...
  async function playFeedback(emotion) {
    ...
    const markCounted = () => {
      ...
      if (!countedCurrentUser) {
        ...
        ui.setCount(count);
        ui.recordEmotion?.({ emotion: result, analysis: latestAnalysis });
      }
    };
    await avatarPlayer.play(result, { loop: false, muted: false, fallbackMs: RESULT_FALLBACK_MS, onStarted: markCounted });
    markCounted();
  ...
  async function playGoodbyeAndReset() {
    ...
    await avatarPlayer.play('goodbye', { loop: false, muted: true, fallbackMs: GOODBYE_FALLBACK_MS });
    await startIdle();
  }
  ...
  return {
    ...
    async start() { running = true; await startIdle(); },
    stop,
    resetToIdle,
    simulate
  };
