...
export function createAvatarPlayer({ video, placeholder, onSoundBlocked, onSoundUnlocked }) {
  ...
  async function play(kind, { loop = false, muted = true, fallbackMs = RESULT_FALLBACK_MS, onStarted } = {}) {
    const candidates = toCandidates(AVATAR_VIDEOS[kind]);
    const playToken = nextToken();
    let started = false;

    function markStarted(payload) {
      if (started || playToken !== token) return;
      started = true;
      onStarted?.(payload);
    }
    ...
    for (const src of candidates) {
      ...
      const played = await playVideoWithSoundFallback({ src, muted, playToken, getToken: () => token });
      if (playToken !== token) return { ok: false, reason: 'interrupted' };
  ...
  async function playVideoWithSoundFallback({ muted, playToken, getToken }) {
    try {
      await video.play();
      return { ok: true, muted };
    } catch (error) {
      if (muted || playToken !== getToken()) return { ok: false, error };
      onSoundBlocked?.({ error });
      video.muted = true;
      try {
        await video.play();
  ...
  return {
    ...
    showPlaceholder,
    unlockSound,
    get soundUnlocked() { return soundUnlocked; }
  };
}
...
function waitForVideoEnd(video, timeoutMs, playToken, getToken) {
  return new Promise(resolve => {
    ...
    const checkInterrupted = () => {
      if (playToken !== getToken()) { finish(); return; }
      if (!done) requestAnimationFrame(checkInterrupted);
    };
...
function waitIfCurrent(ms, playToken) {
  return new Promise(resolve => { setTimeout(() => resolve(playToken), ms); });
}