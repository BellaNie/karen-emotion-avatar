...
function normalizeJournal() {
  ...
  if (!stored || stored.date !== date || !Array.isArray(stored.records)) {
    const fresh = { date, records: [] };
    writeStoredJournal(fresh);
    return fresh;
  }
  const normalized = { date, records: stored.records.map(normalizeRecord).slice(0, MAX_RECORDS) };
...
function findDominantEmotion(counts) {
  return EXPRESSION_KEYS.reduce((best, key) => {
    if (!best || counts[key] > counts[best]) return key;
    return best;
  }, 'neutral');
}
...
export function createEmotionJournal() {
  return {
    getSummary() { return buildSummary(normalizeJournal()); },
    add({ emotion, analysis } = {}) {
      ...
      const nextJournal = { date: journal.date, records: [record, ...journal.records].slice(0, MAX_RECORDS) };
