...
function averageExpressions(samples) {
  if (!samples.length) {
    return emptyExpressionAverages();
  }
  ...
  samples.forEach(sample => {
    EXPRESSION_KEYS.forEach(key => {
      totals[key] += Number(sample[key] || 0);
    });
  });
...
function getWinner(averages) {
  return EXPRESSION_KEYS.reduce((best, key) => {
    if (!best || averages[key] > averages[best]) {
      return key;
    }
...
function classifyAverages(averages, validSampleCount) {
  ...
  if (
    CLASSIFICATION.elevatedEmotions.includes(winner) &&
    confidence < CLASSIFICATION.elevatedThreshold
  ) {
    return {
      emotion: winner,
      winner,
      ...
      confidence,
      margin,
      reason: 'elevated-threshold'
    };
  }

  return {
    emotion: winner || 'neutral',
    winner,
    runnerUp,
    confidence,
    margin,
...
function buildAnalysis({
  averages = emptyExpressionAverages(),
  samples = [],
  validSampleCount = 0,
  ...
  return {
    ...
    async collect({ shouldContinue, onFace, onProgress }) {
      ...
      for (let index = 0; index < DETECTION.expressionSamples; index += 1) {
        ...
        if (index > 0) {
          await sleep(DETECTION.expressionIntervalMs);
        }
